...
AGENT_PROGRESS_EVALUATOR_ENV_ROLES = Sequential(
    NamedBlock(
        refname="agent_progress_env_role",
        name="Environment Roles",
        content=Sequential(
            "Here are the different roles in the environment:",
            Collection(
                ...
                NamedVariable(
                    refname="evaluator",
                    name="Evaluator",
                    content="The evaluator who assesses the progress of the plan that {agent} has proposed, equipped with tools.",
                ),
            ),
            "You are the {evaluator}.",
        ).set_sep("\n"),
...
AGENT_SS_EVALUATOR_ENVIRONMENT_DESC = NamedBlock(
    "Environment",
    Collection(AGENT_SS_EVALUATOR_ENV_ROLES, ENV_SETUP,)
...
AGENT_HELP_EVALUATOR_ENVIRONMENT_DESC = NamedBlock(
    "Environment",
    Collection(AGENT_HELP_EVALUATOR_ENV_ROLES, ENV_SETUP,)
    .set_indexing_method(sharp3_indexing)
    .set_sep("\n\n"),
)
